...
class ManaPro_FilterSuperSlider_Model_Observer extends Mage_Core_Helper_Abstract {
	...
	public function processUpdate($observer) {
		...
		switch ($object->getEntityName()) {
			case 'mana_filters/filter2':
				if (!Mage::helper('mana_db')->hasOverriddenValue($object, $values, 9)) {
					$object->setSliderNumberFormat($this->_getDefaultSliderNumberFormat());
				}
                if (!Mage::helper('mana_db')->hasOverriddenValue($object, $values, 24)) {
			...
			case 'mana_filters/filter2_store':
                ...
                if (!Mage::helper('mana_db')->hasOverriddenValue($object, $values, 10)) {
                    $object->setSliderManualEntry($values['slider_manual_entry']);
                }
                if (!Mage::helper('mana_db')->hasOverriddenValue($object, $values, 11)) {
                    $object->setSliderNumberFormat2($values['slider_number_format2']);
                }
                if (!Mage::helper('mana_db')->hasOverriddenValue($object, $values, 12)) {
                    $object->setSliderThreshold($values['slider_threshold']);
                }
                if (!Mage::helper('mana_db')->hasOverriddenValue($object, $values, 13)) {
                    $object->setSliderUseExistingValues($values['slider_use_existing_values']);
                }
                if (!Mage::helper('mana_db')->hasOverriddenValue($object, $values, 22)) {
                    $object->setSliderDecimalDigits($values['slider_decimal_digits']);
                }
                if (!Mage::helper('mana_db')->hasOverriddenValue($object, $values, 23)) {
    ...
    public function addEditedData($observer) {
        ...
        switch ($object->getEntityName()) {
            case 'mana_filters/filter2':
            case 'mana_filters/filter2_store':
                Mage::helper('mana_db')->updateDefaultableField($object, 'slider_number_format', 9, $fields, $useDefault);
                Mage::helper('mana_db')->updateDefaultableField($object, 'slider_manual_entry', 10, $fields, $useDefault);
                Mage::helper('mana_db')->updateDefaultableField($object, 'slider_number_format2', 11, $fields, $useDefault);
                Mage::helper('mana_db')->updateDefaultableField($object, 'slider_threshold', 12, $fields, $useDefault);
                Mage::helper('mana_db')->updateDefaultableField($object, 'slider_use_existing_values', 13, $fields, $useDefault);
                Mage::helper('mana_db')->updateDefaultableField($object, 'slider_decimal_digits', 22, $fields, $useDefault);
                Mage::helper('mana_db')->updateDefaultableField($object, 'slider_decimal_digits2', 23, $fields, $useDefault);
                Mage::helper('mana_db')->updateDefaultableField($object, 'range_step', 24, $fields, $useDefault);
                break;
        }
    }
